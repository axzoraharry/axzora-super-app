...
/**
 * @title HappyPaisaToken Improved (HP)
 ...
 * IMPROVEMENTS: Zero fees, optimized gas, better stability
 */
contract HappyPaisaTokenImproved is ERC20, Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address _usdtAddress) ERC20("Happy Paisa Token Improved", "HP") Ownable(msg.sender) {
    ...
    /**
     * @dev Owner can deposit additional USDT collateral to improve stability
     ...
     * @param hpAmount Amount of HP tokens to mint
     * @return Required USDT amount (exact amount needed)
     */
     ...
     * @param hpAmount Amount of HP tokens to burn
     * @return USDT amount to be returned (full amount)
     */
    ...
    /**
     * @dev Check if the token is stable and healthy
     * @return isStable Whether the peg is stable (within 2% of target)
     * @return pegDeviation Percentage deviation from target (positive = over-collateralized)
     */
    function checkStability() external view returns (bool isStable, int256 pegDeviation) {
        uint256 totalHPSupply = totalSupply();
        if (totalHPSupply == 0) {
            return (true, 0);
        }
        
        uint256 targetValue = (totalHPSupply * HP_TO_USDT_RATE) / 10**HP_DECIMALS;
        uint256 actualCollateral = totalCollateral;
        
        if (targetValue == 0) {
            return (true, 0);
        }
        
        // Calculate deviation percentage (scaled by 100 for precision)
        if (actualCollateral >= targetValue) {
            pegDeviation = int256(((actualCollateral - targetValue) * 10000) / targetValue);
            isStable = pegDeviation <= 500; // Within 5% over-collateralization
        } else {
            pegDeviation = -int256(((targetValue - actualCollateral) * 10000) / targetValue);
            isStable = pegDeviation >= -200; // Not more than 2% under-collateralized
        }
    }
    
    /**
     * @dev Get comprehensive contract info
     */
    function getContractInfo() external view returns (
        ...
        if (totalSupplyHP == 0) {
            ...
            // Check stability
            (isStable, pegDeviation) = this.checkStability();
        }
    }
    
    /**
     * @dev Emergency function to rebalance collateral (owner only)
     * Can be used to maintain peg stability if needed
     */
    function emergencyRebalance() external onlyOwner {
        uint256 totalHPSupply = totalSupply();
        require(totalHPSupply > 0, "No HP tokens in circulation");
        
        uint256 targetCollateral = (totalHPSupply * HP_TO_USDT_RATE) / 10**HP_DECIMALS;
        uint256 currentCollateral = totalCollateral;
        
        if (currentCollateral < targetCollateral) {
            // Under-collateralized - owner needs to deposit more
            uint256 deficit = targetCollateral - currentCollateral;
            require(
                USDT.transferFrom(msg.sender, address(this), deficit),
                "Emergency deposit failed"
            );
            totalCollateral += deficit;
            emit CollateralDeposited(msg.sender, deficit);
        }
    }
