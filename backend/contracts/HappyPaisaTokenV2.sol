...
/**
 * @title HappyPaisaTokenV2 (HP)
 * @dev A collateral-backed stable token pegged to 1000 INR = 12 USDT
 * The contract maintains a collateral pool of USDT to back the HP tokens
 */
contract HappyPaisaTokenV2 is ERC20, Ownable, ReentrancyGuard, Pausable {
    IERC20 public immutable USDT;
    
    // 1 HP = 1000 INR = 12 USDT (using 6 decimals for USDT precision)
    uint256 public constant HP_TO_USDT_RATE = 12 * 10**6; // 12 USDT (6 decimals)
    ...
    uint256 public reserveRatio = 110; // 110% collateralization ratio (10% over-collateralized)
    ...
    constructor(address _usdtAddress) ERC20("Happy Paisa Token V2", "HP") Ownable(msg.sender) {
    ...
    function mintTokens(uint256 hpAmount) external nonReentrant whenNotPaused {
        ...
        uint256 inrValue = (hpAmount * INR_EQUIVALENT) / 10**(HP_DECIMALS - 3); // Convert to INR with 3 decimals
     ...
     * @param newRatio New reserve ratio (100 = 100%, 110 = 110%, etc.)
     */
    ...
    /**
     * @dev Calculate INR equivalent of HP tokens
     * @param hpAmount Amount of HP tokens
     * @return INR equivalent (with 3 decimal places)
     */
    function calculateINRValue(uint256 hpAmount) external pure returns (uint256) {
        return (hpAmount * INR_EQUIVALENT) / 10**(HP_DECIMALS - 3);
    }
    
    /**
     * @dev Get contract info including INR values
     */
